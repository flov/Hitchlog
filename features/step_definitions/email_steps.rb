...
Then /^(?:I|they|"([^"]*?)") should receive (an|no|\d+) emails? with subject "([^"]*?)"$/ do |address, amount, subject|
  address = User.find_by_username(address).email unless address.include?("@")
...
Then /^(?:I|they|"([^"]*?)") should receive an email with the following body:$/ do |address, expected_body|
  ...
  expect(last_email_sent).to have_subject("[Hitchlog] New Message From #{from_user.capitalize}")
end
...
Then(/^the last email should have been delivered to "(.*?)"$/) do |username|
  expect(last_email_sent).to have_subject('Welcome')
end

Then(/^"(.*?)" should have received an email from "(.*?)"$/) do |to_user, from_user|
  expect(last_email_sent).to have_subject("[Hitchlog] New Message From #{from_user.capitalize}")
end
